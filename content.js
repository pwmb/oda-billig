...
let MAX_ALLOWED_HIGHLIGHT_ITEMS = 4

unitPrices
  .map(e => ({ price: getPriceFor(e), e }))
  .filter(e => e.price !== undefined)
  .sort((a, b) => a.price - b.price)
  .forEach((e, i) => i <= MAX_ALLOWED_HIGHLIGHT_ITEMS ? highlightElement(e.e, i) : "")



function getPriceFor(e) {
  const unitPrice = e.innerText
  const perKgOrLOrPiece = unitPrice.match(/kr ([0-9,]*) per (kg|l|piece|stk)/);
  if (!perKgOrLOrPiece) {
